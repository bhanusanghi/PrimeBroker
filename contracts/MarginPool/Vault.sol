...
interface IVault {
    ...
    event Repay(address indexed creditManager, uint256 borrowedAmount, uint256 interest, uint256 profit, uint256 loss);

    function borrow(address borrower, uint256 amount) external;

    function repay(address borrower, uint256 amount, uint256 interestAccrued)
        // uint256 loss,
        // uint256 profit
        external;
...
contract Vault is IVault, ERC4626 {
    ...
    modifier onlyAdmin() {
        require(aclManager.hasRole(CHRONUX_ADMIN_ROLE, _msgSender()), "Vault: Chronux Admin only");
        _;
    }

    modifier onlyMarginManager() {
        require(aclManager.hasRole(MARGIN_MANAGER_ROLE, _msgSender()), "Vault: Margin manager only");
        _;
    }
    ...
    {
        require(_asset != address(0) && _interestRateModelAddress != address(0), Errors.ZERO_ADDRESS_IS_NOT_ALLOWED);
     ...
     */
    function redeem(uint256 shares, address receiver, address owner) public override(ERC4626) returns (uint256) {
    ...
    function borrow(address borrower, uint256 amount) external override onlyMarginManager {
    ...
    function repay(
        ...
        // transfer
        // if (profit > 0) {
        SafeERC20.safeTransferFrom(IERC20(asset()), borrower, address(this), borrowedAmount.add(interest));
        // .add(profit)
    ...
    function getInterestRateModel() external view override(IVault) returns (address) {
    ...
    // only for test help
    function interestToTime(uint256 amount, uint256 interest) public returns (uint256 time) {
    ...
    function _updateBorrowRate(uint256 loss) internal {
        ...
        borrowAPY_RAY = interestRateModel.calcBorrowRate(expectedLiquidityLastUpdated, totalAssets()); // T:[PS-27]
