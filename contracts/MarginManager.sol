...
import {
    IRiskManager, VerifyTradeResult, VerifyCloseResult, VerifyLiquidationResult
} from "./Interfaces/IRiskManager.sol";
...
contract MarginManager is IMarginManager, ReentrancyGuard {
    ...
    function openMarginAccount() external returns (address) {
        require(marginAccounts[msg.sender] == address(0x0), "MM: Margin account already exists");
        IMarginAccountFactory marginAccountFactory =
            IMarginAccountFactory(contractRegistry.getContractByName(keccak256("MarginAccountFactory")));
        address newMarginAccountAddress = marginAccountFactory.createMarginAccount();
        marginAccounts[msg.sender] = newMarginAccountAddress;
        IMarginAccount(newMarginAccountAddress).setTokenAllowance(vault.asset(), address(vault), type(uint256).max);
    ...
    function borrowFromVault(uint256 amount) external {
        IMarginAccount marginAccount = IMarginAccount(_requireAndGetMarginAccount(msg.sender));
        ...
        returns (uint256 amountOut)
    {
        ...
        ICollateralManager collateralManager =
            ICollateralManager(contractRegistry.getContractByName(keccak256("CollateralManager")));
        require( // required so that approve cannot be called on random malicious erc20s
        collateralManager.isAllowedCollateral(tokenIn), "MM: Invalid tokenIn");
        require(collateralManager.isAllowedCollateral(tokenOut), "MM: Invalid tokenOut");
        if (tokenIn == tokenOut) revert("MM: Same token");
        // swap
        amountOut = _swapAsset(marginAccount, tokenIn, tokenOut, amountIn, minAmountOut);
        // check if health factor is good enough now.
        require(riskManager.isAccountHealthy(address(marginAccount)), "MM: Unhealthy account");
    }
    ...
    function updatePosition(bytes32 marketKey, address[] calldata destinations, bytes[] calldata data) external {
        ...
        VerifyTradeResult memory verificationResult =
            riskManager.verifyTrade(marginAccount, marketKey, destinations, data);
        marginAccount.execMultiTx(destinations, data);
        _executePostMarketOrderUpdates(marginAccount, marketKey, verificationResult);
    }

    function closePosition(bytes32 marketKey, address[] calldata destinations, bytes[] calldata data) external {
        IMarginAccount marginAccount = IMarginAccount(_requireAndGetMarginAccount(msg.sender));
        console2.log("closePosition: ", marginAccount.isActivePosition(marketKey));
        console2.logBytes32(marketKey);
        require(marginAccount.isActivePosition(marketKey), "MM: Trader does not have active position in this market");
        // @note fee is assumed to be in usdc value
        // Add check for an existing position.
        VerifyCloseResult memory result = riskManager.verifyClosePosition(marketKey, destinations, data);
    ...
    function liquidate(
        ...
        IMarginAccount marginAccount = IMarginAccount(_requireAndGetMarginAccount(trader));
        VerifyLiquidationResult memory result =
            riskManager.verifyLiquidation(marginAccount, marketKeys, destinations, data);
        ...
        bool hasBadDebt =
            riskManager.isTraderBankrupt(address(marginAccount), totalBorrowedX18, result.liquidationPenaltyX18);
        _swapAllTokensToVaultAsset(marginAccount);
        if (totalBorrowedX18 > 0) {
            _repayMaxVaultDebt(marginAccount, totalBorrowedX18, interestAccruedX18);
        }
        if (!hasBadDebt) {
            ...
            marginAccount.transferTokens(
                vault.asset(),
                result.liquidator,
                result.liquidationPenaltyX18.convertTokenDecimals(18, IERC20Metadata(vault.asset()).decimals())
            );
        } else {
        ...
        emit AccountLiquidated(address(marginAccount), result.liquidator, result.liquidationPenaltyX18);
    }

    // ---------------- Internal and private functions --------------------

    function _requireAndGetMarginAccount(address trader) private view returns (address) {
        require(marginAccounts[trader] != address(0), "MM: Invalid margin account");
    ...
    function _executePostMarketOrderUpdates(
        ...
        Position memory marketPosition = riskManager.getMarketPosition(address(marginAccount), marketKey);
    ...
    function _executePostPositionCloseUpdates(IMarginAccount marginAccount, bytes32 marketKey) private {
        Position memory marketPosition = riskManager.getMarketPosition(address(marginAccount), marketKey);
        require(marketPosition.size == 0 && marketPosition.openNotional == 0, "MM: Invalid close position call");
        require(riskManager.isAccountHealthy(address(marginAccount)), "MM: Unhealthy account");
    }
    ...
    function _swapAsset(
        ...
        amountOut = marginAccount.swapTokens(tokenIn, tokenOut, amountIn, minAmountOut);
    }

    function _swapAllTokensToVaultAsset(IMarginAccount marginAccount) private {
        ICollateralManager collateralManager =
            ICollateralManager(contractRegistry.getContractByName(keccak256("CollateralManager")));
        address[] memory allowedCollaterals = collateralManager.getAllCollateralTokens();
    ...
    // }
    function _repayMaxVaultDebt(IMarginAccount marginAccount, uint256 totalBorrowedX18, uint256 interestAccruedX18)
        private
    {
        uint256 vaultAssetBalance = IERC20(vault.asset()).balanceOf(address(marginAccount));
        ...
        uint256 interestAccrued = interestAccruedX18.convertTokenDecimals(18, IERC20Metadata(vault.asset()).decimals());
        uint256 totalBorrowed = totalBorrowedX18.convertTokenDecimals(18, IERC20Metadata(vault.asset()).decimals());
    ...
    function _executePostLiquidationUpdates(IMarginAccount marginAccount, bytes32[] memory marketKeys) private {
    ...
    function _verifyPostLiquidation(IMarginAccount marginAccount, VerifyLiquidationResult memory result) private {
        // check if all margin is transferred back to Chronux.
        int256 marginInMarkets = riskManager.getCurrentDollarMarginInMarkets(address(marginAccount));
        require(marginInMarkets == 0, "MM: Margin not transferred back to Chronux");
        // When margin in market is 0 it implies all positions are also closed.
    }

    function _borrowFromVault(IMarginAccount marginAccount, uint256 amount) private {
        riskManager.verifyBorrowLimit(
            address(marginAccount), amount.convertTokenDecimals(IERC20Metadata(vault.asset()).decimals(), 18)
        );
    ...
    function _repayVault(IMarginAccount marginAccount, uint256 amount) private {
        uint256 interestAccrued =
            marginAccount.getInterestAccruedX18().convertTokenDecimals(18, IERC20Metadata(vault.asset()).decimals());
        require(amount + interestAccrued > 0, "MM: repaying 0 amount not allowed");
        ...
        for (uint256 i = 0; i < traders.length; i += 1) {
            if (IERC20(_token).balanceOf(marginAccounts[traders[i]]) > 0) {
                IMarginAccount(marginAccounts[traders[i]]).transferTokens(
                    _token, owner, IERC20(_token).balanceOf(marginAccounts[traders[i]])
                );
            }
        }
    }

    function setRiskManager(address _riskmgr) external nonZeroAddress(_riskmgr) onlyOwner {
        riskManager = IRiskManager(_riskmgr);
    }

    function setVault(address _vault) external nonZeroAddress(_vault) onlyOwner {
        vault = IVault(_vault);
    }
}
