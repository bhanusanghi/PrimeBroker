...
describe("MarginManager", () => {
  ...
  describe("Fork test", () => {
    ...
    it.only("should allow opening a long position on perp directly", async () => {
      ...
      expect(staticResponse.quote).to.be.eq(parsedAmount);
      ...
      // expect(response.base).to.be.eq(staticResponse.base)
      // expect(response.quote).to.be.eq(parsedAmount);
