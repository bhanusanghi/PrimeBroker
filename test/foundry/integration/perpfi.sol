...
contract PerpfiIntegration is BaseSetup {
    ...
    function testMarginInMarket(uint margin) public {
        ...
        int256 expectedMarginValueX18 = contracts.priceOracle.convertToUSD(
            perpMargin,
            usdc
        ) * 10 ** 12;
        int256 marginFromChronux = contracts
            .perpfiRiskManager
            .getDollarMarginInMarkets(bobMarginAccount);
        int256 marginTpp = perpfiUtils.fetchMargin(
            bobMarginAccount,
            perpAaveKey
        );
    ...
    function testPositionSize(int256 positionSize) public {
        ...
        (int256 perpPositionSize, int256 perpPositionOpenNotional) = perpfiUtils
            .fetchPosition(bobMarginAccount, perpAaveKey);
        Position memory position = IMarginAccount(bobMarginAccount).getPosition(
            perpAaveKey
        );
    ...
    function testPositionNotional(int256 positionNotional) public {
        ...
        (int256 perpPositionSize, int256 perpPositionNotional) = perpfiUtils
            .fetchPosition(bobMarginAccount, perpAaveKey);
        Position memory position = IMarginAccount(bobMarginAccount).getPosition(
            perpAaveKey
        );
    ...
    function testPositionOrderFee() public {
        ...
        uint256 markPrice1 = perpfiUtils.getMarkPrice(
            perpExchange,
            perpMarketRegistry,
            perpAaveMarket
        );
        ...
        int256 unrealisedPnL = contracts.perpfiRiskManager.getUnrealizedPnL(
            bobMarginAccount
        );
    ...
    function testUnrealisedPnL(int256 openNotional) public {
        ...
        (
            ...
            uint256 pendingFee
        ) = IAccountBalance(perpAccountBalance).getPnlAndPendingFee(
                bobMarginAccount
            );
        assertEq(
            unrealisedPnL,
            owedRealizedPnl + unrealizedPnl + int256(pendingFee),
            "Unrealised PnL does not match"
        );
    }
