...
contract Liquidate_MarginManager_UnitTest is MarginManager_UnitTest {
    function test_liquidate_when_invalid_trader() public invalidMarginAccount {
        ...
        contracts.marginManager.liquidate(david, params.activeMarkets, params.destinations, params.data);
    }
    ...
    function test_liquidate_when_final_margin_in_TPP_not_zero() public {
        ...
        (address destination, bytes memory data) = chronuxUtils.getPerpfiClosePositionData(perpAaveKey);
    ...
    function test_liquidate_when_trader_has_vault_liability_no_bad_debt() public {
        ...
        utils.simulateUnrealisedPnLSnx(
            circuitBreaker, bobMarginAccount, snxUni_marketKey, openPosition.openNotional, openPosition.size, -280 ether
        );
        LiquidationParams memory params = chronuxUtils.getLiquidationData(bob);
        ...
        vm.mockCall(
            bobMarginAccount,
            abi.encodeWithSelector(IMarginAccount.getInterestAccruedX18.selector),
            abi.encode(interestX18)
        );
        uint256 interest = interestX18 / 1e12;
        (bool isLiquit,, uint256 liqPenalty) = contracts.riskManager.isAccountLiquidatable(bobMarginAccount);
        assertEq(isLiquit, true);

        liqPenalty = liqPenalty / 1e12;
        (int256 accruedFunding,) = IFuturesMarket(market).accruedFunding(bobMarginAccount);
        uint256 initialBalanceAlice = IERC20(usdc).balanceOf(alice);
        uint256 initialBalanceVault = IERC20(usdc).balanceOf(address(contracts.vault));
        uint256 accValue = contracts.riskManager.getAccountValue(bobMarginAccount);
        (uint256 orderFee,) = IFuturesMarket(market).orderFee(positionSize);
        vm.prank(alice);
        contracts.marginManager.liquidate(bob, params.activeMarkets, params.destinations, params.data);
        assertEq(IERC20(susd).balanceOf(bobMarginAccount), 0, "balance of susd should be 0");
        assertApproxEqAbs(
            IERC20(usdc).balanceOf(alice), initialBalanceAlice + liqPenalty, 1 * ONE_USDC, "liquidator was not paid"
        );
        assertApproxEqAbs(
            int256(IERC20(usdc).balanceOf(bobMarginAccount)),
            int256((accValue / 1e12)) - int256(liqPenalty) - (accruedFunding / 1e12) - int256(orderFee / 1e12),
            3 * ONE_USDC,
        ...
        assertEq(
            contracts.collateralManager.getFreeCollateralValue(bobMarginAccount) / 1e12,
