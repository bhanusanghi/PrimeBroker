...
contract PerpfiUtils is Test, Constants, IEvents {
    ...
    function getMarkPrice(address exchange, address perpMarketRegistry, address _baseToken)
        public
        view
        returns (uint256 token0Price)
    {
        uint160 val = IExchange(exchange).getSqrtMarkTwapX96(_baseToken, 0);
        return formatX96ToX10_18(formatSqrtPriceX96ToPriceX96(IExchange(exchange).getSqrtMarkTwapX96(_baseToken, 0)));
    }

    function fetchMargin(address marginAccount, bytes32 marketKey) public returns (int256 margin) {
        margin = int256(IVault(perpVault).getBalance(marginAccount));
    }

    function getAccountValue(address marginAccount) public returns (int256 accountValue) {
        accountValue = int256(IVault(perpVault).getAccountValue(marginAccount));
    }

    function verifyMarginOnPerp(address marginAccount, bytes32 marketKey, int256 expectedMargin) public {
    ...
    function fetchPosition(address marginAccount, bytes32 marketKey)
        public
        returns (int256 positionSize, int256 positionOpenNotional)
    {
        address baseToken = contracts.marketManager.getMarketBaseToken(marketKey);
        int256 marketSize = IAccountBalance(perpAccountBalance).getTakerPositionSize(marginAccount, baseToken);
        int256 marketOpenNotional = IAccountBalance(perpAccountBalance).getTotalOpenNotional(marginAccount, baseToken);
        positionSize = marketSize;
        positionOpenNotional = -marketOpenNotional;
    }

    function verifyPositionSize(address marginAccount, bytes32 marketKey, int256 expectedPositionSize) public {
        // Position memory positionChronux = IMarginAccount(marginAccount).getPosition(marketKey);
        (int256 marketPositionSize, int256 marketPositionNotional) = fetchPosition(marginAccount, marketKey);
        assertEq(marketPositionSize, expectedPositionSize, "expectedPositionSize not equal to chronux position size");
        // assertEq(positionChronux.size, marketPositionSize, "Market Position Size not equal to chronux position size");
    }

    function verifyPositionNotional(address marginAccount, bytes32 marketKey, int256 expectedPositionNotional) public {
        ...
        // );
        assertApproxEqAbs(
            expectedPositionNotional, marketPositionNotional, DUST_THRESHOLD, "Perp position notional does not match"
        );
    }
    ...
    function updateAndVerifyPositionSize(
        ...
        tradeData.marketAddress = contracts.marketManager.getMarketAddress(tradeData.marketKey);
        tradeData.marginAccount = contracts.marginManager.getMarginAccount(trader);
        tradeData.baseAsset = contracts.marketManager.getMarketBaseToken(tradeData.marketKey);
        ...
        if (shouldFail) {
            ...
            contracts.marginManager.updatePosition(tradeData.marketKey, tradeData.txDestinations, tradeData.txData);
            verifyPositionSize(
                tradeData.marginAccount, tradeData.marketKey, deltaPositionSize + tradeData.initialPositionSize
            );
        }
    ...
    function updateAndVerifyPositionNotional(
        ...
        if (!shouldFail) {
            ...
            verifyPositionNotional(
                tradeData.marginAccount, tradeData.marketKey, deltaPositionNotional + tradeData.initialPositionNotional
            );
        }
        vm.stopPrank();
    }
    ...
    function updateAndVerifyMargin(
        ...
        int256 marginX18Value = contracts.priceOracle.convertToUSD(marginX18, usdc);
        vm.startPrank(trader);
        ...
        int256 freeCollateralPerp = int256(IVault(perpVault).getFreeCollateral(marginAccount));
        data[0] = abi.encodeWithSignature("approve(address,uint256)", perpVault, margin);
        data[1] = abi.encodeWithSignature("deposit(address,uint256)", usdc, margin);
        if (shouldFail) {
            vm.expectRevert(reason);
            ...
            if (margin > 0) {
                ...
                emit MarginTransferred(marginAccount, marketKey, usdc, marginX18, marginX18Value);
                contracts.marginManager.updatePosition(marketKey, destinations, data);
                verifyMarginOnPerp(marginAccount, marketKey, currentMargin + margin);
                ...
                data[0] = abi.encodeWithSignature("withdraw(address,uint256)", usdc, margin.abs());
                vm.expectEmit(true, true, true, true, address(contracts.marginManager));
                emit MarginTransferred(marginAccount, marketKey, usdc, marginX18, marginX18Value);
                contracts.marginManager.updatePosition(marketKey, destinations, data);
                verifyMarginOnPerp(marginAccount, marketKey, freeCollateralPerp + margin);
            }
        }
        vm.stopPrank();
    }
    ...
    function closeAndVerifyPosition(address trader, bytes32 marketKey) public {
        ...
        data[0] = abi.encodeWithSelector(0x00aa9a89, baseToken, 0, 0, type(uint256).max, bytes32(0));
        // check event for position opened on our side.
        vm.expectEmit(true, true, true, true, address(contracts.marginManager));
    ...
    function revertClosePosition(address trader, bytes32 marketKey, bytes calldata errorMessage) public {
        vm.startPrank(trader);
        address marketAddress = contracts.marketManager.getMarketAddress(marketKey);
        address baseToken = contracts.marketManager.getMarketBaseToken(marketKey);
        address marginAccount = contracts.marginManager.getMarginAccount(trader);
